! function($, api) {
    api.sectionConstructor.upsell = api.Section.extend({
        ...
        initCustomControl: function() {
            "use strict";
            var control = this;
            control.populate(control), control.container.on("click", ".btn-add-field", control, function(event) {
                control.add(event)
                ...
                control.remove(event), control.updateValue()
            })
        },
        populate: function() {
            "use strict";
            var wrapper = this.selector,
                multi_saved_value = jQuery(wrapper).find(".repeater-text-value").val();
            if (multi_saved_value.length > 0) {
                var multi_saved_values = multi_saved_value.split("|");
                jQuery(wrapper).find(".repeater-text-fields").empty(), jQuery.each(multi_saved_values, function(i, value) {
        ...
        add: function(event) {
            "use strict";
            event.preventDefault(), jQuery(event.target).parent(".repeater-text-input").find(".repeater-text-fields").append('<div class="set"><input type="text" value="" class="repeater-text-single-field" /><span class="btn-remove-field"><span class="dashicons dashicons-no-alt"></span></span></div>')
        },
        remove: function(event) {
            "use strict";
            event.preventDefault(), jQuery(event.target).parent().parent().remove()
        },
